...
- Session 8: Komplettes Landing Page Redesign – Hero mit AnimatedBG + Designer-Dashboard-Mockup (Preiskalkulation animiert), WhyWBC Tabelle → 6 Metric-Cards (dark section), interaktiver PricingCalculator mit Custom Slider, ProblemSolution Bento-Layout (dark), HowItWorks Lucide Icons statt Emojis, AnimateOnScroll mit 5 Animationstypen (fade-up/left/right/scale-in/blur-in). Neue Unterseiten: /features (SEO, Hover-Redesign mit farbigen Accent-Bars), /preise (3 Pläne + Feature-Tabelle + FAQ), /datenschutz, /impressum. Nav full-width + Opacity-Fix (bg-white/96 gegen dunkle Sektionen). Footer komplett neu (dark, 3-Spalten, Brand+CTA+Trust). Build fehlerfrei. Push auf GitHub/Vercel. Nächste Schritte: CSV-Export, Testdaten mit Lisa & Soraya, Produktbilder-Upload.
