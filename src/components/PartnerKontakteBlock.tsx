...
import {
  kontaktAnlegen,
  kontaktAktualisieren,
  kontaktLoeschen,
  kontaktAlsHauptkontaktSetzen,
  type KontaktDaten,
} from '@/app/actions/partner'
import type { PartnerKontakt } from '@/lib/supabase/types'
...
function leereDaten(): KontaktDaten {
  return {
    name: '',
    rolle: null,
    email: null,
...
export default function PartnerKontakteBlock({
  partnerId,
  initialKontakte,
}: {
  partnerId: string
  initialKontakte: PartnerKontakt[]
}) {
  const [kontakte, setKontakte]   = useState<PartnerKontakt[]>(initialKontakte)
  const [modalOffen, setModalOffen] = useState(false)
  const [bearbeite, setBearbeite] = useState<PartnerKontakt | null>(null)
  ...
  function bearbeitenOeffnen(k: PartnerKontakt) {
    setBearbeite(k)
    setFehler(null)
    setModalOffen(true)
  }

  function alsHauptSetzen(k: PartnerKontakt) {
    ...
    startTransition(async () => {
      const r = await kontaktAlsHauptkontaktSetzen(k.id, partnerId)
      if (r.fehler) {
        setFehler(r.fehler)
        // optimistisches Update revertieren
        setKontakte((prev) =>
  ...
  function loeschen(id: string) {
    startTransition(async () => {
      const r = await kontaktLoeschen(id, partnerId)
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <h2 className="text-sm font-semibold text-gray-900">
          Kontaktpersonen <span className="text-gray-400 font-normal">({kontakte.length})</span>
        </h2>
        <button
          onClick={neuOeffnen}
      ...
      {kontakte.length === 0 ? (
        <div className="text-center py-12 px-4">
          ...
          <p className="text-sm font-medium text-gray-700">Noch keine Kontaktpersonen</p>
          <p className="text-xs text-gray-400 mt-1 mb-4">Lege den ersten Ansprechpartner an, damit du gezielt anrufen oder E-Mails verschicken kannst.</p>
          <button
            onClick={neuOeffnen}
      ...
      <ConfirmModal
        ...
        message="Diese Kontaktperson wird dauerhaft entfernt. Andere Daten zum Partner bleiben erhalten."
        confirmText="Löschen"
        isLoading={isPending}
...
function KontaktModal({
  ...
  partnerId:        string
  initial:          PartnerKontakt | null
  istErsterKontakt: boolean
  onClose:          () => void
  onAngelegt:       (k: PartnerKontakt) => void
  onAktualisiert:   (k: PartnerKontakt) => void
}) {
  // Erster Kontakt = automatisch Hauptkontakt
  const [daten, setDaten] = useState<KontaktDaten>(() => {
    if (initial) {
      return {
        name:             initial.name,
        rolle:            initial.rolle,
  ...
  function speichern() {
    ...
    startTransition(async () => {
      if (initial) {
        const r = await kontaktAktualisieren(initial.id, partnerId, daten)
        ...
        const r = await kontaktAnlegen(partnerId, daten)
        if (r.fehler) { setFehler(r.fehler); return }
        // Wir kennen die generierte ID nicht direkt; das nächste Server-Refetch
        // ersetzt diese Optimistic-ID. Hier reicht ein temporärer Wert.
          ...
          <label className="flex items-start gap-2.5 p-3 border border-gray-200 rounded-lg bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors">
            ...
            <div className="flex-1">
              ...
              <p className="text-[11px] text-gray-500 mt-0.5">
                Der Hauptkontakt wird in Listen, PDFs und der Übersicht prominent angezeigt. Pro Partner gibt es genau einen.
              </p>
            </div>
          </label>
        </div>
